...
** Builtin bindings that I want to remember

| Key    | Mode   | Action           | Instead of                 |
|--------+--------+------------------+----------------------------|
| =gwip= | Normal | Refill paragraph | Visual select then do =gq= |
* Fundamental
** Be iMproved
...
On my linode machine, I got errors because the following listchars use
unicode characters.  With =LC_ALL= unset, vim would say "too many characters
for =extends= probably because that little =›= is more than one byte.  This
does not happen when =LC_ALL= is set to something like =en_CA.UTF-8=.
...
The default is just ~wildmode=full~ which is annoying because if you have
...
hello_z_long_suffix
#+end_src
then doing =:tabe he<TAB>= will complete to =:tabe hello_a_long_suffix= and
if you wanted =hello_z_long_suffix=, then you can press TAB 25 times (or in
this case SHIFT-TAB once) or backspace till you get =:tabe hello_=, then add
a 'z', and finally press TAB and open the file.

With =longest=, VIM will only complete the longest common prefix of the
current candidate list.

Keeping =full= in there makes the =wildmenu= continue to pop up.
...
For systems where I don't want to bring in plugins and all that stuff, here
is a quick list of the builtin themes organized in categories:

| Type                              | Themes                                       |
|-----------------------------------+----------------------------------------------|
| White background                  | delek, morning, zellner                      |
| Grey background lower contrast    | desert, evening, habamax, lunaperche, slate  |
| Black background, higher contrast | elflort, industry, koehler, torte, wildcharm |
...
(*): The version has to be recent enough I think.
...
- [[https://invisible-island.net/xterm/ctlseqs/ctlseqs.html#h3-Operating-System-Commands][XTerm control sequences (the defacto standard)]]
...
#+begin_src vimrc :tangle ftplugin/c.vim
...
#+begin_src vimrc :tangle ftplugin/html.vim
...
** Notes about the =clipboard= setting for OSCYank

While experimenting with OSCYank on various machines: MacOS and Linux (at
work always accessed over SSH without X Forwarding).  I changed stuff to make
it work on one machine then it wouldn't work on the other and vice versa.

Changing the =clipboard= setting would make a difference until I ended up
using register 0 in the function for =MyOSCYankAndNormalYank()=.

I'm keeping the following sections but they are no longer relevant and none
of their SRC blocks go into my vimrc.

*** Value of =clipboard= and choice of register

Right now I have register =getreg("0")= used in the function.  This seems to work
everywhere.

I started out with =getger("+")= however.  With that setting, I needed to
have ~set clipboard=unnamed~.  When I tried this at work on Windows in an SSH
connection to a Linux machine it didn't work giving the message

#+begin_src sh :tangle no
'W23: Clipboard register not available, using register 0'
#+end_src

I think this is because there is no X server.  So I did this:
...
and later I had problems that were resolved by using register 0 no matter
what which is just simpler.

*** Addition for MacOS/local

I only use vim on Linux through an SSH connection and locally on MacOS so I
don't know which it is.

Vim 'OSCYank' doesn't seem to work on local MacOS but adding this:

#+begin_src vimrc :tangle no
set clipboard=unnamed
#+end_src

either makes OSCYank work or has Vim set my system clipoard.  When I get back on
Linux, I'll have to see if this doesn't break anything.

See [[https://vim.fandom.com/wiki/Accessing_the_system_clipboard][Vim Fandom Accessing the System Clipboard]].  It talks about "+" and "*" being
the same on some systems and different on others.
