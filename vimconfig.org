...
*** Preventing accidental number increments
...
This section tangles to files =ftplugin/<FILETYPE>.vim= for various values of
=FILETYPE=.

Using =<buffer>= in commands like =nnoremap <buffer> KEY ACTIONS= makes it so
=KEY= is bound to =ACTION= only in the particular buffer.  This is important
because without it, the =<Space>sb= would be the one for the most recently
opened filetype in all buffers.
...
nnoremap <buffer> <Leader>ssb ggi#!/bin/bash<CR><BS><CR>set -euEo pipefail<CR>shopt -s inherrit_errexit<CR><ESC>
